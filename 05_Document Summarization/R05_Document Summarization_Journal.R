...
# Load the data
TPAMI <- read.csv("IEEE_TPAMI.csv", encoding = "UTF-8", stringsAsFactors = FALSE)
JoF <- read.csv("Journal of Finance.csv", encoding = "UTF-8", stringsAsFactors = FALSE)
...
myStopwords <- c(stopwords("SMART"), "financ", "american", "associ", "firm",
                 "model", "data", "algorithm", "method", "imag")
...
wordcloud(TPAMI.wcdat$word, TPAMI.wcdat$freq, min.freq=3, scale = c(5, 0.2), rot.per = 0.1, col=pal, random.order=F)
...
pal <- brewer.pal(8, "Dark2")
wordcloud(JoF.wcdat$word, JoF.wcdat$freq, min.freq=3, scale = c(5, 0.2), rot.per = 0.1, col=pal, random.order=F)

# Association Rules for IEEE_TPAMI
TPAMI.tran <- as.matrix(t(TPAMI.TDM))
TPAMI.tran <- as(TPAMI.tran, "transactions")

TPAMI.rules <- apriori(TPAMI.tran, parameter=list(minlen=2,supp=0.1, conf=0.85))
inspect(TPAMI.rules)
...
# Association Rules for Journal of Fiance
JoF.tran <- as.matrix(t(JoF.TDM))
JoF.tran <- as(JoF.tran, "transactions")

JoF.rules <- apriori(JoF.tran, parameter=list(minlen=2, supp=0.06, conf=0.8))
inspect(JoF.rules)
...
freq.idx <- which(rowSums(TPAMI.wcmat) >= 10)
...
TPAMI.graph <- delete.edges(TPAMI.graph, which(E(TPAMI.graph)$weight <= 15))
...
# For Journal of Finance Abstracts
# Change it to a Boolean matrix
JoF.wcmat[JoF.wcmat >= 1] <- 1
# find the words that are used more than 10 times
freq.idx <- which(rowSums(JoF.wcmat) >= 10)
...
JoF.graph <- delete.edges(JoF.graph, which(E(JoF.graph)$weight <= 10))
